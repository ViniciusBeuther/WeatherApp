...
export default function WeatherComponent(props) {
    ...
    const firstForecast = props.data.list && props.data.list.length > 0 ? props.data.list[0] : null;

    return (
        <>
            <div>
                <span>
                    {firstForecast && firstForecast.weather && firstForecast.weather[0] ? (
                        //<img src={chooseIcon(firstForecast.weather[0].icon)} alt="weatherIcon" />
                        <img src={chooseIcon('04d')} alt="weatherIcon" />
                    ) : (
                        <div></div>
                    )}
                </span>
                <span>
                    <strong><h1>{firstForecast ? firstForecast.main.temp : 'N/A'}</h1></strong> °C
                </span>
            </div>
        </>
    );
